...
import { readClient, readManifest, allSybils, gql, REPO_ROOT } from "@pof/core";
...
const d = await gql<{ agents: { id: string; reviewCount: number; reviewAvgBps: number; honoredCount: number; proofOfFillScore: string }[] }>(
  `{ agents(where:{reviewCount_gte:20}){ id reviewCount reviewAvgBps honoredCount proofOfFillScore } }`);
...
for (const a of d.agents) {
  console.log(`  ${a.id.slice(0, 10)}  ★${(a.reviewAvgBps / 10000).toFixed(2)} from ${a.reviewCount} reviewers` +
    `  →  ${a.honoredCount} fills, Proof-of-Fill score ${a.proofOfFillScore}`);
}
