...
contract ProofOfFillScoreTest is Test {
    ...
    function _s(uint128 usd6, uint32 h, uint32 f, uint16 div)
        internal pure returns (ProofOfFillScore.Score memory)
    {
        return ProofOfFillScore.Score({
            honoredValueUsd6: usd6, honoredCount: h, failedCount: f,
            diversityBps: div, updatedAt: 0
        });
    }
    ...
    function test_DiversityScalesLinearly() public view {
        ...
        assertEq(score.computeScore(_s(31_240e6, 12, 0, 8_320)), 25_991);
    }

    /// Failures weigh 3x: 10 honored / 1 failed -> 10/(10+3) = 10/13
    function test_FailureWeightingIsThreeToOne() public view {
        uint32 clean = score.computeScore(_s(1_300e6, 10, 0, 10_000));
        uint32 dirty = score.computeScore(_s(1_300e6, 10, 1, 10_000));
        assertEq(clean, 1_300);
        assertEq(dirty, 1_000, "1300 * 10/13 == 1000");
    ...
    function test_SmallHonestBeatsLargeReneger() public view {
        uint32 honest  = score.computeScore(_s(1_000e6, 5, 0, 8_000));
        uint32 reneger = score.computeScore(_s(10_000e6, 2, 8, 7_500));
    ...
    function test_OnlyAttestorCanSet() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(
            IAccessControl.AccessControlUnauthorizedAccount.selector, outsider, roleId
        ));
    ...
    function test_SetAndReadRoundTrip() public {
        _set(alice, 31_240e6, 12, 0, 8_320);
    ...
    function testFuzz_NeverExceedsRawValue(uint128 usd6, uint32 h, uint32 f, uint16 div) public view {
        ...
        if (base > type(uint32).max) base = type(uint32).max;
